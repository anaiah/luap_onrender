...
router.post('/xapi/postimage', upload.single('uploaded_file') ,  async (req, res) => {
	...
	if(path.extname(req.file.originalname)===".jpg" || path.extname(req.file.originalname)==='.jpeg'){
		extname = ".jpg"
	}else{
					...
					if(results[1].length>0){
						...
						QRCode.toFile( 'x.png', hostname, {
							...
							if (err){
								...
								let xclient,xaddress,xfone,xemail,xdata
								...
								.then((res) => {
									...
									fs.access(xpath, fs.F_OK, (err) => {
										...
										//if file exists fire email
										//================== CREATE EMAIL ================//
										let transporter = nodemailer.createTransport({
											
											service: 'gmail',
											auth: {
												user:'anaiahdaniel@gmail.com',
												pass: 'ritauxpagwnnhbbw'
											},
											
											
											/*
											host: 'mail.vantaztic.com',
											port:465,
											auth: {
												user: 'system@vantaztic.com',
												pass: '(F6B1}!EOE'
											},
											*/
										...
										let htmlTemp = `Dear Vantaztic Operations,  <br><br>
											Please find attached DR Pdf file for PO# ${req.params.po_number}</b>
											that is "approved" for "release".<br><br>
											<font color=red>PLS. DO NOT REPLY, THIS IS A SYSTEM GENERATED EMAIL.</font>
											<br><br><br><br>
											<img src='http://www.vantaztic.com/vanz/vantaztic_logo.png' height='39px'><br>
											<font size='1px'>
											5117 Mindanao Ave. Ext, Bgy. Ugong, Valenzuela City<br>
											Tel Nos. (02) 703-4252; (02) 7 500-9039<br>
											Email : sales@vantaztic.com<br>
											http://www.vantaztic.com 
											</font>
											`

										const mailOptions = {
											from: 'system@vantaztic.com', // sender address
											to: 'carlo.dominguez@vantaztic.com', // list of receivers
											subject: `PO # ${req.params.po_number} Approved for Release`, // Subject line
											html: htmlTemp, // plain text body
											attachments: [{
												filename: `DR_${req.params.po_number}.pdf`,
												path: path.join(__dirname, '..',`/dr/DR_${req.params.po_number}.pdf`),
	...
	.then((db)=>{
		...
		db.query(sql, null ,(error,data) => {	
			if(data.length >0){
				...
				res.send(`<font color=red>PO ${req.params.po_number} OK FOR RELEASE!</font>`)
			}else{
				res.send(`<font color=red>ERROR! PO ${req.params.po_number} WAS ALREADY RELEASED!!!</font>`)
			}
		})
	}).catch((error)=>{
		res.status(500).json({error:'No Fetch Docs'})
